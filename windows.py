...
def get_value_pokaz():
    ...
    def show_pokaz():
        print(pokaz[3:])
        label_for_show_pokaz = Label(window2, text=f"{pokaz[3][0]}->{pokaz[3][1]}\n{pokaz[4][0]}->{pokaz[4][1]}\n{pokaz[5][0]}->{pokaz[5][1]}")
...
btn_for_input_pokaz = Button(window2, text="Ввести количество показателей", command=get_value_pokaz)
btn_for_input_pokaz.place(relx=0.014, rely=0.30)

#  //--------------------Добавление названий вершинам--------------------\\
label_for_create_e = Label(window2, text="Ввести ребра")
